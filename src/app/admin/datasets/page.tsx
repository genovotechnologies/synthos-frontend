                ...
                <div className="px-5">
                  {data.datasets.map((d) => (
                    <div key={d.id} className="grid grid-cols-12 gap-4 py-3 border-b border-zinc-800/30 last:border-b-0 items-center text-sm">
                      ...
                      <div className="col-span-3 text-zinc-500 tabular-nums">{formatBytes(d.file_size)}</div>
      ...
      {pagination && pagination.total_pages > 1 && (
        <div className="flex items-center justify-between pt-4">
          <span className="text-sm text-zinc-600">
            Page {pagination.page} of {pagination.total_pages} ({pagination.total} datasets)
          </span>
