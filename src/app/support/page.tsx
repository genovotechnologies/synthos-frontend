...
// Window of most-recent tickets used for the derived (urgent/high/unassigned) counts.
const TICKET_WINDOW = 50;
...
export default function SupportOverviewPage() {
  ...
  const { data: ticketsData, isLoading: ticketsLoading, isError: ticketsError } = useQuery({
    queryKey: ['support', 'tickets', 'recent-window'],
    queryFn: () => supportApi.listTickets(1, TICKET_WINDOW),
    retry: 1,
  });
  ...
  const windowSize = Math.min(tickets.length, TICKET_WINDOW);
  const urgentCount = tickets.filter((t) => t.priority === 'urgent').length;
  const highCount = tickets.filter((t) => t.priority === 'high').length;
  const unassignedCount = tickets.filter((t) => !t.assigned_to).length;

  // Global counts from the overview endpoint.
  const stats = [
    ...
    { label: 'Waiting', value: overview?.tickets_by_status?.waiting ?? 0 },
  ...
  return (
    <div className="space-y-16">
      ...
      <section>
        {overviewError ? (
          <InlineLoadError label="Failed to load ticket status counts. Refresh the page to try again." />
        ) : (
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-x-12 gap-y-10">
            {stats.map((stat) => (
              <div key={stat.label}>
      ...
      {/* Urgency + Unassigned Indicators — scoped to the most recent ticket window */}
      <section>
        <p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-4">
          ...
          <span className="ml-2 text-zinc-600 normal-case tracking-normal">
            in last {windowSize || TICKET_WINDOW} tickets
          </span>
        </p>
        {ticketsError ? (
          <InlineLoadError label="Failed to load recent tickets. Refresh the page to try again." />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {urgentCount > 0 && (
              <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-5">
                ...
                <p className="text-xs text-rose-400/60 mt-1">in the last {windowSize} tickets need immediate attention</p>
              </div>
            )}
            {highCount > 0 && (
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-5">
                ...
                <p className="text-xs text-amber-400/60 mt-1">in the last {windowSize} tickets awaiting resolution</p>
              </div>
            )}
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-5">
              ...
              <p className="text-xs text-blue-400/60 mt-1">of the last {windowSize || TICKET_WINDOW} tickets need assignment</p>
            </div>
          </div>
        )}
      </section>
